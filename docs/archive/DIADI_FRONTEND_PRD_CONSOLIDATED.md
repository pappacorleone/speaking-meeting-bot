# Diadi Frontend PRD (Consolidated)
...
Sources:
- docs/PRD_DIADI_FRONTEND.md
- docs/DIADI_FRONTEND_PRD.md
- docs/DYADIC_ALPHA_MASTER_STRATEGY.md
- docs/Diadi screens/*

## Table of Contents
1. Executive Summary
2. Comparison and Consolidation Notes
3. Strategy Alignment and Constraints
4. Target Users and Jobs
5. Scope and Phasing
6. Experience Architecture
7. Screen Inventory and Coherence Decisions
8. Functional Requirements (Alpha)
9. Data Model and State
10. Real-Time Events and Intervention Logic
11. Analytics and Success Metrics
12. Non-Functional Requirements
13. Implementation Guide Using Existing Codebase
...
## 2. Comparison and Consolidation Notes
This PRD merges the two drafts and resolves differences:
- Stack specificity: DIADI_FRONTEND_PRD.md specifies Next.js 14; PRD_DIADI_FRONTEND.md is stack-agnostic. Consolidated decision: Next.js 14 + React + TypeScript is the recommended default; teams may adapt.
- Session creation steps: DIADI_FRONTEND_PRD.md defines 6 steps (includes Strategic Intent and Emotional Pulse). PRD_DIADI_FRONTEND.md defines a leaner 5-step flow. Consolidated decision: Alpha uses the lean flow; Strategic Intent and Emotional Pulse are P1 optional steps.
- Native video vs external platform: DIADI_FRONTEND_PRD.md includes WebRTC in Phase 3. PRD_DIADI_FRONTEND.md positions external platforms as alpha default. Consolidated decision: Alpha uses external platform integration; native Diadi video is Beta.
- Event stream: DIADI_FRONTEND_PRD.md proposes /ws/{client_id}/feedback; PRD_DIADI_FRONTEND.md proposes /sessions/{id}/events. Consolidated decision: use /sessions/{id}/events with internal mapping to client_id; /ws/{client_id}/feedback is acceptable as an internal alias.
- Deep prep: DIADI_FRONTEND_PRD.md includes rehearsal and prep; PRD_DIADI_FRONTEND.md defers to Beta. Consolidated decision: Deep Prep is Beta; keep placeholders in Alpha.
...
### 7.1 Screen Inventory by Folder (actual files)
...
### 8.2 Session Creation Wizard (Alpha)
Core steps:
1. Identity and bond
2. Session goal
...
### 8.8 Profile and Integrations (Minimal)
Acceptance criteria:
...
### 13.1 Current Backend Capabilities (this repo)
| Endpoint | Purpose | Notes |
| --- | --- | --- |
| POST /bots | Create a MeetingBaas bot | Requires x-meeting-baas-api-key and meeting_url |
...
### 13.2 Backend Additions Required for the UX
...
### 13.3 Real-Time Event Stream
Preferred:
- WebSocket /sessions/{session_id}/events

Acceptable internal alias:
- WebSocket /ws/{client_id}/feedback (map session_id to client_id server-side)

### 13.4 Frontend Architecture (recommended)
Stack:
- Next.js 14 + React + TypeScript
- Tailwind + component primitives (e.g., shadcn/ui)
...
### 13.6 Alpha Build Path with Current Backend
If backend changes are not ready:
- UI asks for meeting_url directly and calls POST /bots.
- Use mock data for consent, summary, and interventions.
- Add a dev toggle to simulate balance updates.
